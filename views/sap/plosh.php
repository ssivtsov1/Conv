...
<h1 align="center">Площадки вимірювання</h1>
<?php
...
<div class="form-group">
    <h4>відсутні площадки вимірювання:</h4>
    <?php
//        debug($kol);
//        return 1;
    ...
    <table id="tbl_rez"  cellpadding="0" cellspacing="0" border=1>
        <tr class="tbl_row">
            ...
            <th class="tbl_header"> кількість площадок</th>
            ...
            <?php if(!empty($s)) {
                for($i=0;$i<count($s);$i++) {
                    ...
                    echo('<td>' . ($i+1) . '</td>');
                    echo('<td>' . $s[$i]['lic_schet'] . '</td>');
                    echo('<td>' . $s[$i]['short_name'] . '</td>');
                    echo('<td>' . $s[$i]['kol'] . '</td>');
                    echo('</tr>');
            }}
          ?>
