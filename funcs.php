...
//        $region='DNP';
        $region=trim($v['reg_wo']);;
        $iuru_pro=$v['numobl'];
        if(!empty($v['id_wo']) || !empty($v['street_wo'])) {
            if(!empty($house_num1)) {
                 $str_supll2 = $house_num1;
                 $house_num1='';
...
// Функция поиска подстроки в строке всех подстрок (регистрозависимая)
// работает на основе алгоритма Рабина - Карпа
function find_str_all($s,$f)
{
    $h = hash_f($f);  // Определяеь хэш подстроки
    $y = strlen($s);
    $yf = strlen($f);
    $j=0;
    for ($i = 0; $i < $y; $i++) {
...
// Рекурсивная функция перестановки
...
function is_elem($mas,$v) {
//    debug($mas);
    $y=count($mas);
    $flag=0;
    for($i=0;$i<$y;$i++) {
//        if(!isset($mas[$i])) continue;
        if(!isset($mas[$i]))
        {
           continue;
        }
